...
@Keep data class PluginResponseRoot(val plugins: List<PluginResponse>)
@Keep data class PluginResponse(val id: String, val name: String, val url: String, val version: String, val active: Boolean)

private val TAG = "PluginManagerUtils"

fun getPluginFromManifest(filePath: String, url: String, version: String, active: Boolean): Plugin? {
    return try {
...
suspend fun checkAndDownloadNewPlugins(downloadUrl: String, remotePlugins: List<PluginResponse>, pluginDao: PluginDao, deletedPlugin: DeletedPluginDao, outputDir: String) {
    ...
    remotePlugins.forEach { remotePlugin ->
        if (localPlugins.none { it.id == remotePlugin.id.lowercase() } && !deletedPlugins.contains(remotePlugin.id.lowercase())) {
            ...
            if (result) {
                val newPlugin = getPluginFromManifest(filePath, downloadUrl, remotePlugin.version, remotePlugin.active)
...
suspend fun downloadPlugins(url: String, pluginDao: PluginDao, outputDir: String) {
    ...
    response.plugins.forEach {
        ...
        val result = downloadPlugin(uri, filePath)
        if (result) {
            val plugin = getPluginFromManifest(filePath, url, it.version, it.active)
            plugin?.let { pl ->
